...
export const WantedPoster: React.FC<{ type: GoblinType }> = ({ type }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-950 p-3 sm:p-4 font-serif">
      {/* 9:16 Poster Canvas */}
      <div ref={posterRef} className="relative w-full max-w-md bg-[#e3d5b8] shadow-[20px_20px_60px_rgba(0,0,0,0.8)] border-[6px] sm:border-[12px] border-[#3d2b1f] overflow-hidden group flex flex-col">
        ...
        {/* Poster Header */}
        <div className="pt-6 sm:pt-10 pb-3 sm:pb-4 text-center">
          <h1 className="text-4xl sm:text-6xl font-black text-[#3d2b1f] tracking-tighter uppercase leading-none italic">
            WANTED
          </h1>
          <div className="mt-2 h-1 w-3/4 mx-auto bg-[#3d2b1f]" />
          <p className="mt-2 text-base sm:text-xl font-bold text-[#3d2b1f] border-y border-[#3d2b1f] inline-block px-3 sm:px-4">
            {data.name}
          </p>
        </div>

        {/* Image Section */}
        <div className="relative mx-4 sm:mx-8 aspect-square border-4 border-[#3d2b1f] overflow-hidden bg-[#171717] group-hover:scale-[1.02] transition-transform duration-500">
          {imgError ? (
        ...
        {/* Poster Body */}
        <div className="px-4 sm:px-8 mt-4 sm:mt-6 space-y-3 text-[#3d2b1f]">
          <div>
        ...
        {/* Poster Footer */}
        <div className="px-4 sm:px-8 pt-3 sm:pt-4 pb-6 sm:pb-8 mt-auto flex justify-between items-end">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase font-bold tracking-widest opacity-60">Threat Level</span>
            <span className="text-2xl sm:text-3xl font-black text-[#7f1d1d]" style={{ filter: 'drop-shadow(0 1px 1px rgba(0,0,0,0.05))' }}>{data.rank}</span>
          </div>
      ...
      <button 
        onClick={handleShare}
        disabled={sharing}
        className="mt-6 sm:mt-8 px-8 sm:px-12 py-3 bg-red-900 text-white font-bold tracking-widest uppercase hover:bg-red-800 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-wait w-full sm:w-auto max-w-md text-sm sm:text-base"
      >
        {sharing ? '生成通緝令中...' : '📸 下載通緝令圖片'}
      </button>

      {/* Social Share Buttons */}
      <div className="mt-4 flex flex-col items-center gap-3 w-full max-w-md px-3 sm:px-0">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-[#06C755] text-white text-xs sm:text-sm font-bold rounded hover:brightness-110 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-[#1877F2] text-white text-xs sm:text-sm font-bold rounded hover:brightness-110 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-[#000000] text-white text-xs sm:text-sm font-bold rounded hover:brightness-150 transition-all border border-neutral-700 cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-neutral-800 text-white text-xs sm:text-sm font-bold rounded hover:bg-neutral-700 transition-all border border-neutral-600 cursor-pointer"
          >
